...
```json
{
  ...
  "request": { "limit": 50 },
  "result": { "items": [], "page": { "limit": 50, "offset": 0, "has_more": false } },
...
* `limit`: integer 1–500; default 50. The adapter adds `-l <limit>` only where supported.
...
| `machines.list` — overview, machines/status | `machines list -o json` when accepted; otherwise fixed human-output parser is not permitted | JSON array of machine records only | `{}`; page mode `none` unless verified command provides pagination | Read; refresh machines/status | Verified command group; structured output is environment-dependent. Empty list is valid. |
| `machines.prune` — machines/status | `machines prune` plus optional fixed `--duration <duration>` and `--not-validated-only`; never `--force` from browser | Mutation result; then `machines.list` | `{duration?, not_validated_only?}`; duration must be ≥2m when supplied | Mutation; explicit confirmation because it removes multiple stale registrations; refresh machines | **MVP included only as environment-dependent.** The command has an interactive safety guard and may fail non-interactively; map that to `unsupported` or `crowdsec_failure`. No arbitrary duration or `--force`. |
...
| `bouncers.delete` — bouncers | `bouncers delete <name>` | Mutation result; then `bouncers.list` | `{name}` | Mutation; explicit confirmation; refresh bouncers | **MVP included only if command exists and target is local.** Never accept or display the bouncer token. If delete is unsupported, omit the control. |
| `hub.list` — scenarios/collections overview | `hub list -o raw` | CSV rows with fixed columns `name,status,version,description,type`; reject malformed rows and unexpected columns | `{type?: enum(parser,scenario,postoverflow,context,collection,appsec-config,appsec-rule)}`; page mode `none` | Read; refresh component views | Verified raw output for hub inventory. `type` is a local filter applied after parsing, not a shell filter. |
| `scenarios.list` — scenarios | `scenarios list -o json` when accepted | JSON array of scenario records | `{}`; page mode `none` | Read; refresh scenarios | Environment-dependent structured output; no enable/disable control is inferred from listing. |
...
| `collections.list` — collections | `collections list -o json` when accepted | JSON array of collection records | `{}`; page mode `none` | Read; refresh collections | Environment-dependent structured output. |
...
| `allowlists.create` — allowlists | `allowlists create <name>` plus optional fixed `-d <description>` | Mutation result; then `allowlists.list` | `{name, description?}`; description 1–256 chars | Mutation; explicit confirmation; refresh allowlists | **MVP included for local allowlists.** Console-managed distinction must prevent mutation of remote entries. |
| `allowlists.add` — allowlists | `allowlists add <name> <ip_or_range>` plus optional fixed `-e <duration>` and `-d <description>` | Mutation result; then `allowlists.list` and `decisions.list` because adding may delete existing decisions | `{name, ip_or_range, expiration?, description?}`; no pagination | Mutation; explicit confirmation; refresh allowlists and decisions | **MVP included for local allowlists.** Expiration must use the duration grammar. Never accept CSV paths or import flags. |
...
| `metrics.show` — overview, optional statistics | `metrics show acquisition -o json`, `metrics show appsec -o json`, or `metrics show lapi -o json` | JSON object shape is component-specific and must be schema-validated per component | `{component: enum(acquisition,appsec,lapi)}`; page mode `none` | Read; refresh only the affected metric card | **Optional/environment-dependent.** Metrics are ordinary CrowdSec data, not Prometheus/Grafana. Unsupported components are reported honestly. |
...
**Reviewer status:** Pending target-environment execution and CrowdSec-domain sign-off. Until sign-off, version-dependent rows must remain capability-gated.
